...
do
    ...
    do
        ...
        do
            ...
            finetune="/home/oturgut/PyTorchEEG/mae_he/mae/output/correct/pre/"$folder"/"$subf"/pre_"$pre_data"/checkpoint-249.pth"
        
            for bs in "${batch_size[@]}"
            do
                for lr in "${blr[@]}"
                do
                    output_dir="./output/correct/lin/"$folder"/"$subf"/lin_b"$bs"_blr"$lr"_"$pre_data
                    log_dir="./logs/correct/lin/"$folder"/"$subf"/lin_b"$bs"_blr"$lr"_"$pre_data
