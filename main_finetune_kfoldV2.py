...
def main(args):
    ...
    csv_filename = f"benchmark-mae_BATCH{args.batch_size}_BLR{args.blr}_dataset-lemon.csv"
...
#############################################################################################################
    def custom_collate_fn(batch, mean, std):
        # Extract samples and labels from the batch
        samples, labels, label_masks = zip(*batch)
        ...
        # Normalize labels
        labels_normalized = (labels - mean) / (std + 1e-8)

        return samples, labels_normalized, label_masks
    ...
    if args.k_folds > 1:
        ...
        # Use the existing split without k-fold cross-validation.
        # Create a dummy "split" that keeps the original train/val datasets.
        splits = [(
            np.arange(len(dataset_train)),  # Indices for training
            np.arange(len(dataset_val)) + len(dataset_train)  # Indices for validation, adjusted to correct range
        )]
    ...
    for fold, (train_idx, val_idx) in enumerate(splits):
        ...
        if args.wandb:
            ...
            if wandb.run is not None:
                wandb.finish() # Make sure prev run is finished
        ...
        # Use partial to create a collate function with predefined mean and std
