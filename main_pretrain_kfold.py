...
def get_args_parser():
    ...
    parser.add_argument('--base_dir', default='',
                        help='path where to save (for k_fold cases), empty for no saving')
...
def main(args):
    ...
    # Prepare data
    dataset = SignalDataset(data_path=args.data_path, train=True, args=args)  # Initially set train=True to load all data
    ...
    if args.online_evaluation:
        dataset_online = SignalDataset(data_path=args.data_path_online, labels_path=args.labels_path_online,
                                            labels_mask_path=args.labels_mask_path_online,
                                            label_map_path=args.label_map_path,
                                            downstream_task=args.online_evaluation_task, train=True, args=args)
        splits_online = prepare_datasets(dataset_online, args.k_folds, args)


###################################  K-FOLD CROSS VALIDATION  ###############################################
    for fold, ((train_idx, val_idx), (train_idx_online, val_idx_online)) in enumerate(zip(splits, splits_online)):
        #Set output directory
        directory_name = os.path.join(args.base_dir, f"Fold{fold}_BATCH{args.batch_size}_BLR{args.blr}")
        ...
        if args.wandb:
            ...
            wandb.init(project=args.wandb_project, config=config,
                    resume=False, id=None, group=f"Pretrain_{args.k_folds}Fold_BATCH{args.batch_size}_BLR{args.blr}",
                    name=f"Pretrain_Fold_{fold}", entity="begum-soenmez")

        # Subset the full dataset into train and validation datasets for the current fold
        dataset_train_fold = SignalDataset(data_path=args.data_path, train=True, indices=train_idx, args=args)
        dataset_val_fold = SignalDataset(data_path=args.data_path, train=False, indices=val_idx, args=args)
        ...
        # DataLoader setup
        if args.online_evaluation:
            dataset_online_train_fold = SignalDataset(data_path=args.data_path_online, labels_path=args.labels_path_online,
                                                labels_mask_path=args.labels_mask_path_online,
                                                label_map_path=args.label_map_path,
                                                downstream_task=args.online_evaluation_task, train=True, indices=train_idx_online, args=args)

            dataset_online_val_fold = SignalDataset(data_path=args.data_path_online, labels_path=args.labels_path_online,
                                            labels_mask_path=args.labels_mask_path_online,
                                            label_map_path=args.label_map_path,
                                            downstream_task=args.online_evaluation_task, train=False, indices=val_idx_online, args=args)

            # Get train label statistics for normalization
            train_labels_online = dataset_online_train_fold.labels
            val_labels_online = dataset_online_val_fold.labels

            args.train_labels_mean = train_labels_online.mean(dim=0)
            args.train_labels_std = train_labels_online.std(dim=0)
        ...
        num_tasks = misc.get_world_size()
        global_rank = misc.get_rank()
        sampler_train = torch.utils.data.DistributedSampler(dataset_train_fold, num_replicas=num_tasks, rank=global_rank, shuffle=True)
        sampler_val = torch.utils.data.SequentialSampler(dataset_val_fold)
        ...
        if args.online_evaluation:
            ...
            baseline_metrics_train = calculate_baseline_metrics(train_labels_online, args.train_labels_mean)
            baseline_metrics_val = calculate_baseline_metrics(val_labels_online, args.train_labels_mean)
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            print(f"Loss / Normalized CC of the network on the {len(dataset_val_fold)} val images: {val_stats['loss']:.4f}\
                / {val_stats['ncc']:.2f}")
